...
public class StationTramwayHandler implements HttpHandler {
    ...
    public StationTramwayHandler(String fusekiEndpoint) {
    ...
    // GET method to retrieve tram stations
    ...
    // POST method to create a new tram station
    private void handlePost(HttpExchange exchange) throws IOException {
        ...
        String insertQuery = "PREFIX ns: <" + ONTOLOGY_NAMESPACE + "> " +
                "INSERT DATA { " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> a ns:StationTramway . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:Id \"" + id + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:NomStation \"" + nomStation + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:Adresse \"" + adresse + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:NuméroDeTéléphone \"" + numeroDeTelephone + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:HorairesOuverture \"" + horairesOuverture + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:HorairesFermeture \"" + horairesFermeture + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "StationTramway" + id + "> ns:CapacitéMax \"" + capaciteMax + "\" . }";

        try {
            executeUpdate(insertQuery);
            sendResponse(exchange, createResponse("Tram Station created: " + nomStation), 201);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to create tram station: " + e.getMessage()), 500);
        }
    }

    // PUT method to update an existing tram station
    private void handlePut(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            sendResponse(exchange, createResponse("Tram Station updated: " + nomStation), 200);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to update tram station: "+ e.getMessage()), 500);
        }
    }

    // DELETE method to remove a tram station
    private void handleDelete(HttpExchange exchange) throws IOException {
        String path = exchange.getRequestURI().getPath();
        String stationId = path.substring(path.lastIndexOf('/')+1); // Extracting the Tram Station ID from URI

        String deleteQuery = 	"PREFIX ns: <"+ ONTOLOGY_NAMESPACE+"> DELETE { <"+ ONTOLOGY_NAMESPACE+"StationTramway"+stationId+"> a ns:StationTramway . } WHERE { <"+ ONTOLOGY_NAMESPACE+"StationTramway"+stationId+"> a ns:StationTramway . }";

        try {
            executeUpdate(deleteQuery);
            sendResponse(exchange, createResponse("Tram Station deleted"), 200);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to delete tram station: "+ e.getMessage()), 500);
        }
    }
