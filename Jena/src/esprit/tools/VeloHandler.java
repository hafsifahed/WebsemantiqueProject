...
public class VeloHandler implements HttpHandler {
    ...
    public VeloHandler(String fusekiEndpoint) {
    ...
    // GET method to retrieve bicycles
    private void handleGet(HttpExchange exchange) throws IOException {
        String queryString = "PREFIX ns: <" + ONTOLOGY_NAMESPACE + "> " +
                "SELECT ?velo ?id ?marque ?modele ?couleur ?immatriculation ?annee ?poids " +
                "WHERE { " +
                "?velo a ns:Velo ; " +
                "ns:Id ?id ; " +
                "ns:Marque ?marque ; " +
    ...
    // POST method to create a new bicycle
    private void handlePost(HttpExchange exchange) throws IOException {
        ...
        double poids = jsonObject.get("poids").getAsDouble();

        String insertQuery = "PREFIX ns: <" + ONTOLOGY_NAMESPACE + "> " +
                "INSERT DATA { " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> a ns:Velo . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Id \"" + id + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Marque \"" + marque + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Modele \"" + modele + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Couleur \"" + couleur + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Immatriculation \"" + immatriculation + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Annee \"" + annee + "\" . " +
                "<" + ONTOLOGY_NAMESPACE + "Velo" + id + "> ns:Poids \"" + poids + "\" . }";

        try {
            executeUpdate(insertQuery);
            sendResponse(exchange, createResponse("Bicycle created: " + marque), 201);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to create bicycle: " + e.getMessage()), 500);
        }
    }

    // PUT method to update an existing bicycle
    private void handlePut(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            sendResponse(exchange, createResponse("Bicycle updated: " + marque), 200);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to update bicycle: "+ e.getMessage()), 500);
        }
    }

    // DELETE method to remove a bicycle
    private void handleDelete(HttpExchange exchange) throws IOException {
        String path = exchange.getRequestURI().getPath();
        String veloId = path.substring(path.lastIndexOf('/')+1); // Extracting the Bicycle ID from URI

        String deleteQuery = 	"PREFIX ns: <"+ ONTOLOGY_NAMESPACE+"> DELETE { <"+ ONTOLOGY_NAMESPACE+"Velo"+veloId+"> a ns:Velo . } WHERE { <"+ ONTOLOGY_NAMESPACE+"Velo"+veloId+"> a ns:Velo . }";

        try {
            executeUpdate(deleteQuery);
            sendResponse(exchange, createResponse("Bicycle deleted"), 200);
        } catch (IOException e) {
            sendResponse(exchange, createResponse("Failed to delete bicycle: "+ e.getMessage()), 500);
        }
    }
